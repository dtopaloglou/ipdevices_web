...
$(function() {
    ...
    var evs = {
        ...
        groups: {
            ...
            pass: "uPassword uCPassword uCurrentPassword",
        },
        errorPlacement: function(error, element) {

            if (
                element.attr("name") == "uFirstname" ||
                element.attr("name") == "uLastname"
            ) {
                error.insertAfter($("#uLastname").closest('.row'));
                element.parents("div.form-group")
                    .addClass('has-error');
            }
            else if (element.attr("name") == "uPassword" ||  element.attr("name") == "uCPassword" || element.attr("name") == "uCurrentPassword")
            {
                console.log(element);
                error.insertAfter($("#uCPassword").closest('.row'));
        ...
        messages: {
            uFirstname: "Please enter firstname and lastname",
            uLastname: "Please enter firstname and lastname",
            uUsername : {
        ...
        submitHandler : function (form)
        {
            ...
            $.ajax({
                ...
                success : function (data)
                {
                    ...
                    $('#edit-profile-results').html(data);
                    console.log(data);
                    $clicker.text(originalText);
                },
                complete : function ()
    ...
    $.extend(evs, config.validations);

    $('#EditProfile').validate(evs);
})
    ...
    var signupValidationSettings = {
		...
		errorPlacement: function(error, element) {
			if (
				...
				element.attr("name") == "cPassword"
			) {
				error.insertAfter($("#cPassword").closest('.row'));
				element.parents("div.form-group")
				.addClass('has-error');
			}
...
function activateMenuLi(_class)
{
    $('#sidebar-menu li').each(function (i, v)
    {
...
$('#myModal').imgPicker({
    ...
    url: '../bower_components/imgpicker/server/upload_avatar_ipdevices.php',
    aspectRatio: 1,
    ...
    loadComplete: function(image) {
        ...
        console.log(image);
        if (image.versions) {
            $('#profile-avatar').attr('src', image.versions.avatar.url +time() );
            $('#small-avatar').css('background-image', "url(" + image.versions.avatar.url +time() + ")" );
            this.setImage(image);
        }
        else
        {
            $('#profile-avatar').attr('src', '../assets/a200.png');
            $('#small-avatar').attr('src', '../assets/a30.png');
        }
    },
    ...
    cropSuccess: function(image) {
        // Set #avatar src
        $('#profile-avatar').attr('src', image.versions.avatar.url);
        $('#small-avatar').css('background-image', "url(" + image.versions.small.url   + ")" );
        // Hide modal
        this.modal('hide');
    }
});
...
$('#profile-avatar').mouseover(function ()
{
    ...
    $(this).addClass('edit-avatar');

}).mouseout(function ()
{
    $(this).removeClass('edit-avatar');
});